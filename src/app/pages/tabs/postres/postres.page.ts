...
export class PostresPage implements OnInit, OnDestroy {
    ...
    this.subsDatos = this.busqueda.busqueda().subscribe((data: string) => {
      if (data != null && data != "" && data != "all") {
        ...
        for (let i = 0; i < this.dataPostres.length; i++) {
          if (this.dataPostres[i].nombre.includes(data)) {
  ...
  private async observadorProductos() {
    ...
    this.subCategorias = this.productosServices.onservarCategorias().subscribe(
      data => {
        if (data) {
          this.categoria = this.productosServices.mapaCategorias.get(environment.nombresCategorias.postres).idCategoria;
