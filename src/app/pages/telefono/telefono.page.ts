...
export class TelefonoPage implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private firebaseAuthentication: FirebaseAuthentication, public alertController: AlertController) { }
  ...
  askCode() {
    this.firebaseAuthentication.verifyPhoneNumber('+593' + this.phoneNumber, 30000).then(verificationId => {
     ...
     // this.validateAlert(verificationId)
    })
  }

  // async validateAlert(verificationId) 
  async validateAlert() {
        ...
        buttons: [
          ...
          }, 
          {
            ...
            handler: (response) => {
              ...
              //this.firebaseAuthentication.signInWithVerificationId(verificationId,smsCode).then(res => alert(response))
            }
          }
        ]
      });
